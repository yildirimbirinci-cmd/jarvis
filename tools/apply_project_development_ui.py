...
IMPORT_LINE = (
    "from artmach_assistant.core.project_development_ui "
    "import install_main_window_project_development"
)
INSTALL_LINE = "install_main_window_project_development(MainWindow)"
MARKER = "# Jarvis project development dashboard integration."
REQUIRED_METHODS = {
    "run_worker",
    "busy",
    "on_proposal_ready",
...
REQUIRED_FILES = (
    "core/project_development_ui.py",
    "core/project_development_dashboard.py",
    "core/project_launch_service.py",
)
...
def _parse(source: str, path: Path) -> ast.Module:
    try:
        ...
        raise RuntimeError(f"app.py sözdizimi geçersiz; entegrasyon uygulanmadı: {exc}") from exc
...
def inspect(project_root: Path) -> Inspection:
    ...
    missing_files = tuple(
        relative for relative in REQUIRED_FILES if not (app_path.parent / relative).is_file()
    )
    return Inspection(
        app_path=app_path,
...
def _patched_source(source: str, app_path: Path) -> str:
    ...
    if missing:
        raise RuntimeError(
            "app.py beklenen GUI yöntemlerini içermiyor; hiçbir dosya değiştirilmedi. Eksik: "
            + ", ".join(missing)
        )
    newline = "\r\n" if "\r\n" in source else "\n"
    ...
    if IMPORT_LINE not in patched or INSTALL_LINE not in patched:
        raise RuntimeError("Proje geliştirme entegrasyonu doğrulanamadı.")
    return patched
...
def _backup_path(app_path: Path) -> Path:
    return app_path.parent / ".jarvis_backups" / "app.py.before_project_development_ui"
...
def apply(project_root: Path) -> Inspection:
    ...
    if not before.compatible:
        details = [
            *(f"app.py yöntemi: {item}" for item in before.missing_methods),
            *(f"dosya: {item}" for item in before.missing_files),
        ]
        raise RuntimeError(
            "Proje geliştirme arayüzü güvenli biçimde uygulanamadı. Eksikler: "
            + "; ".join(details)
        )
    if before.installed:
        return before
    ...
    if not after.installed:
        ...
        raise RuntimeError("Son doğrulama başarısız; app.py geri alındı.")
    return after
...
def revert(project_root: Path) -> Path:
    ...
    if not backup.is_file():
        raise FileNotFoundError(f"Geri alma yedeği bulunamadı: {backup}")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Jarvis proje geliştirme sekmesini güncel app.py üzerine güvenli ekler."
    )
...
def main() -> int:
    ...
    try:
        ...
        if result.missing_methods:
            print("eksik yöntemler: " + ", ".join(result.missing_methods))
